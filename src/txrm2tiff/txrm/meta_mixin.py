...
from ..xradia_properties.enums import XrmDataTypes, XrmCameraType, XrmObjectiveType, XrmSourceType

class MetaMixin:
    
    @txrm_property(fallback=0)
    def configured_camera_count(self):
        return self.read_stream("ConfigureBackup/ConfigCamera/NumberOfCamera", XrmDataTypes.XRM_UNSIGNED_INT)[0]
    ...
    def _get_detector(self, index):
        ...
        camera_name = self.read_stream(f"{stream_stem}/CameraName", XrmDataTypes.XRM_STRING)[0]
        return model.Detector(
            id=f"Detector:{index}",
            gain=self.read_stream(f"{stream_stem}/PreAmpGain", XrmDataTypes.XRM_FLOAT)[0],
            amplification_gain=self.read_stream(f"{stream_stem}/OutputAmplifier", XrmDataTypes.XRM_FLOAT)[0],
            model=camera_name
        )
        
    @txrm_property(fallback=None)
    def detector(self):
        camera_idx = self.read_stream("ImageInfo/CameraNo", XrmDataTypes.XRM_UNSIGNED_INT)[0] - 1  # Stream from 1
        return self.configured_detectors[camera_idx]

    @txrm_property(fallback=None)
    def microscope(self):
        machine_id = self.read_stream("ConfigureBackup/XRMConfiguration/MachineID", dtype=XrmDataTypes.XRM_STRING)
        kwargs = {}
        if machine_id:
    ...
    def configured_objectives(self):
        return [obj for i in range(self.configured_camera_count) for obj in self._get_objectives(i)]
    ...
    def _get_objectives(self, index):
        ...
        objective_ids = [XrmObjectiveType(id_).name.replace(" ", "_") for id_ in self.read_stream(f"{stream_stem}/ConfigObjectives/ObjectiveID", XrmDataTypes.XRM_UNSIGNED_INT)]
        magnifications = self.read_stream(f"{stream_stem}/ConfigObjectives/OpticalMagnification", XrmDataTypes.XRM_FLOAT)
        return [
            model.Objective(
                id=f"Objective:{id_}",
                nominal_magnification=magnification,
                model=name)
                for id_, name, magnification in zip(objective_ids, objective_names, magnifications)
        ]
    ...
    def configured_source_count(self):
        return self.read_stream("ConfigureBackup/ConfigSources/NumberOfSources", XrmDataTypes.XRM_UNSIGNED_INT)[0]
    ...
    def _get_light_source(self, index):
        stream_stem = f"ConfigureBackup/ConfigSources/Source {index + 1}" # Stream from 1
        source_type = XrmSourceType(self.read_stream(f"{stream_stem}/Type", XrmDataTypes.XRM_UNSIGNED_INT)[0])
    ...
    def light_source(self):
        source_idx = self.read_stream('AcquisitionSettings/SourceIndex', XrmDataTypes.XRM_UNSIGNED_INT)[0] - 1  # Stream from 1
